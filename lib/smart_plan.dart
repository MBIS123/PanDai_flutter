...
class _SmartPlanningPageState extends State<SmartPlanningPage> {
  ...
  Future<void> _fetchFinancialPlan() async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      print(
          'JSON response: $jsonResponse'); // Add this line to print JSON response for debugging
      ...
      print(
          'Failed to load financial plan. Status code: ${response.statusCode}');
  ...
  Future<void> _deleteFinancialPlan(int financialPlanId) async {
    final String apiUrl =
        'http://10.0.2.2:8080/api/v1/financialPlan/deleteFinancialPlan/$financialPlanId';
    ...
    if (response.statusCode == 200) {
      print(
          "Financial plan with ID $financialPlanId has been deleted successfully.");
      // Perform any additional actions upon successful deletion if needed
    } else {
      print(
          'Failed to delete financial plan with ID $financialPlanId. Status code: ${response.statusCode}');
      print('Error response: ${response.body}');
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      ...
                      color: Theme.of(context)
                          .colorScheme
                          .primary, // Set the text color to the primary color from the default color scheme
                    ),
                  ),
                )
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return Dismissible(
                      key: UniqueKey(),
                      // Key to identify the item being dismissed
                          ...
                          builder: (BuildContext context) {
                            return AlertDialog(
                              ...
                              actions: <Widget>[
                                ...
                                TextButton(
                                  onPressed: () {
                                    _deleteFinancialPlan(
                                        financialPlans[index].financialPlanId);
                            ...
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (context) => TempAdvice(
                                  ...
                                  successbilityScore:
                                      financialPlans[index].successScore,
                                  budgetExpenseAdjustment:
                                      financialPlans[index].financialAdjustment,
                                ),
                              ),
                            ).then((_) {
                              _fetchFinancialPlan(); // Call _fetchFinancialPlan() after TempAdvice page is popped
                      ...
                      Navigator.of(context)
                          .push(MaterialPageRoute(
                              builder: (context) => SavingPlanPage(
                                  title: "Saving Plan", userId: widget.userId)))
                          .then((_) {
                        _fetchFinancialPlan(); // Call _fetchFinancialPlan() after TempAdvice page is popped
                      });
                      ;
